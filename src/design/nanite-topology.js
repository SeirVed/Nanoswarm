...
export function analyseNaniteTopology(model, cutoffNm = 0.34) {
  const cells = new Map(); const pairs = []; const degree = new Uint8Array(model.count); const cell = cutoffNm;
  for (let index = 0; index < model.count; index += 1) {
    ...
    for (let dx = -1; dx <= 1; dx += 1) for (let dy = -1; dy <= 1; dy += 1) for (let dz = -1; dz <= 1; dz += 1) for (const other of cells.get(key(x + dx, y + dy, z + dz)) ?? []) {
      ...
      if (distance >= 0.14 && distance <= cutoffNm && pairs.length < 16000) { pairs.push(other, index); degree[other] += 1; degree[index] += 1; }
    }
