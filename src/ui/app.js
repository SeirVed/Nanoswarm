...
function resourcesHtml(now) {
  ...
  const substrate = state.discovery.surveyComplete
    ? `<section class="panel substrate-panel${newUnlockClass("substrate")}" data-unlock-id="substrate" data-tooltip="The active material field is finite; inputs are reserved when collection starts.">
        ...
        <small data-tooltip-key="substrate:inventory" data-tooltip="The surveyed envelope includes every local shell found so far; the remaining percentage and accessible mass apply only to the active field. Collector capacity is the maximum discrete payload one nanite can reserve when a collection job starts.">${state.activeDeposit.cumulativeMass ?? "local shell"} surveyed envelope · active field: ${formatCount(depositTotal)} constituent atoms · ≈${formatInventoryMass(
