...
export function createNaniteViewport(canvas, model, topology, { onSelect, focusedModules = [] } = {}) {
  ...
  const scheduleIdle = () => {
    ...
    state.idleTimer = window.setTimeout(() => {
      const rotate = () => { state.yaw += 0.0015; draw(); state.idleFrame = window.requestAnimationFrame(rotate); };
  ...
  scheduleIdle();
  return { draw, reset, focusModules(indices) { stopIdle(); state.focusedModules = new Set(indices); draw(); scheduleIdle(); }, toggleProjection() { stopIdle(); state.perspective = !state.perspective; draw(); scheduleIdle(); return state.perspective; }, toggleElement(index) { stopIdle(); state.elements.has(index) ? state.elements.delete(index) : state.elements.add(index); draw(); scheduleIdle(); }, toggleBonds() { stopIdle(); state.bonds = !state.bonds; draw(); scheduleIdle(); return state.bonds; }, state };
}
