...
export function createNaniteViewport(canvas, model, topology, { onSelect } = {}) {
  ...
  const draw = () => {
    ...
    if (state.bonds) { const byIndex = new Map(points.map((point) => [point.index, point])); context.strokeStyle = "rgba(137, 255, 170, 0.68)"; context.lineWidth = 1.1; for (let bond = 0; bond < topology.pairs.length; bond += 2) { const left = byIndex.get(topology.pairs[bond]); const right = byIndex.get(topology.pairs[bond + 1]); if (!left || !right) continue; context.beginPath(); context.moveTo(left.x, left.y); context.lineTo(right.x, right.y); context.stroke(); } }
