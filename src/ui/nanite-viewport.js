...
export function createNaniteViewport(canvas, model, { onSelect } = {}) {
  const context = canvas.getContext("2d");
  const state = { yaw: -0.72, pitch: 0.38, panX: 0, panY: 0, zoom: 1, perspective: true, elements: new Set([0, 1, 2, 3]), selected: -1, drag: null, points: [] };
  const draw = () => {
  ...
  return { draw, reset() { Object.assign(state, { yaw: -0.72, pitch: 0.38, panX: 0, panY: 0, zoom: 1 }); draw(); }, toggleProjection() { state.perspective = !state.perspective; draw(); return state.perspective; }, toggleElement(index) { state.elements.has(index) ? state.elements.delete(index) : state.elements.add(index); draw(); }, state };
}
