...
export function createNaniteViewport(canvas, model, topology, { onSelect } = {}) {
  const context = canvas.getContext("2d");
  const state = { yaw: -0.72, pitch: 0.38, panX: 0, panY: 0, zoom: 1, perspective: true, bonds: false, elements: new Set([0, 1, 2, 3]), selected: -1, drag: null, points: [] };
  const draw = () => {
    ...
    for (let index = 0; index < model.count; index += 1) {
      ...
      points.push({ index, x: rect.width / 2 + state.panX + rx * base * factor, y: rect.height / 2 + state.panY - ry * base * factor, depth, radius, element });
    }
    points.sort((left, right) => right.depth - left.depth); state.points = points;
    if (state.bonds) { const byIndex = new Map(points.map((point) => [point.index, point])); context.strokeStyle = "rgba(105, 240, 138, 0.18)"; context.lineWidth = 0.55; for (let bond = 0; bond < topology.pairs.length; bond += 2) { const left = byIndex.get(topology.pairs[bond]); const right = byIndex.get(topology.pairs[bond + 1]); if (!left || !right) continue; context.beginPath(); context.moveTo(left.x, left.y); context.lineTo(right.x, right.y); context.stroke(); } }
    for (const point of points) { const color = NANITE_ELEMENTS[point.element].color; context.beginPath(); context.fillStyle = point.index === state.selected ? "#f7fff8" : color; context.globalAlpha = Math.min(0.92, 0.34 + (10 - point.depth) * 0.07); context.arc(point.x, point.y, point.radius, 0, Math.PI * 2); context.fill(); }
    context.globalAlpha = 1;
    if (state.selected >= 0) { const point = points.find((item) => item.index === state.selected); if (point) { context.strokeStyle = "#ffffff"; context.lineWidth = 1; context.beginPath(); context.arc(point.x, point.y, point.radius + 4, 0, Math.PI * 2); context.stroke(); } }
  };
