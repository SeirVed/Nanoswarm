...
Most throughput branches are authored as additive five-percent refinement series. Their work and resource costs rise by 50% per tier, but tier visibility is also coupled to material-search horizons: one new tier at 0.9 g, 9 g, 90 g, and 900 g. Tier V and later remain unavailable until their environments are authored. Because required work is fixed while reasoning capacity grows with the swarm, the displayed ETA is intentionally dynamic: an initially formidable job can collapse to minutes after several replication rounds without falsifying its duration.
