...
Directed Bond Ablation becomes available after Cohort Ratio Prognostics and a 1,000-nanite observation threshold. It is a special computronium-directed operation rather than a worker cohort. Starting a discharge consumes stored energy, removes an exact proportional mixture from the current finite substrate, and stores that reserved matter inside the active ablation event. Completion transfers the same matter into Feedstock and records it as collected. Online and offline progress process the same completion event. The first local discharge is a calibration wave sized to four recipe-equivalent payloads per active nanite; the second is controlled delamination at sixteen; later saturation fractures release sixty-four. These are matter-handling scales, not promised nanite output: substrate composition, sorting capability, and missing recipe elements still determine what can replicate.

Pipelined Self-Assembly follows the first ablation capability at 10,000 active nanites. It does not make jobs continuous or permit partial nanites. Frame formation, conductor deposition, logic transfer, and calibration overlap within each authoritative cohort, reducing its duration from 55 seconds to approximately 35 seconds while preserving exact recipe reservation and whole completion events.
