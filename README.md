...
- Saves use an explicit version and a BigInt-aware codec. Version 11 adds persistent replication tuning and burst reservations while preserving every earlier migration and the hidden-element ledger without inventing identities for legacy `unknown` atoms.
...
- At 180 nanites, Cohort Ratio Prognostics turns the swarm's growing self-model into a live comparison between current and coherent substrate-conversion ETAs. Completing it exposes the exact replication-efficiency score, current bottleneck, complete-recipe buffer, and potential speed multiplier. Holding 99% efficiency for 30 seconds then enables Temporary Burst, which reserves every whole buffered recipe, deploys it through replication, and restores the exact previous shares and locks.
